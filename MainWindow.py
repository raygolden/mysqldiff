...
class MainWindow (gtk.Window):
    ...
    def compare_dbs(self, db1_host, db1_user, db1_pass, db1_db, db2_host, db2_user, db2_pass, db2_db):
        ...
        # Output table diff:
        if diff == []:
            self.output(" - Table names match each other, no tables missing on either server.")
        else:
            for db in diff:
            ...
            self.output(" ! The table \"%s\" exists only on \"%s\"." % (db, server))
        ...
        for table in db1_tables:
            ...
            if tdata1 == tdata2:
                self.output(" - \"%s\" is the same on both servers." % table)
            else:
                self.output(" ! \"%s\" has differences:" % table)
            
                # Get fieldname lists:
                db1_table = self.get_fieldname_list(tdata1)
                db2_table = self.get_fieldname_list(tdata2)
                
                diff = list(set(db1_table) - set(db2_table))
                
                if diff == []:
                    for field in db1_table:
                        db1_field = self.get_field_from_fieldname(field, tdata1)
                        db2_field = self.get_field_from_fieldname(field, tdata2)
                        
                        # Check datatype:
                        if db1_field[1] != db2_field[1]:
                            self.output("        - Different datatype with field \"%s\": '%s' on '%s', '%s' on '%s'." % (field, db1_field[1], db1.get_host(), db2_field[1], db2.get_host()))
                        ...
                        if db1_field[2] != db2_field[2]:
                            ...
                            self.output("        - Field \"%s\" on \"%s\" is %s whereas on \"%s\" %s." % (field, db1.get_host(), db1_null, db2.get_host(), db2_null))
                    ...
                    for field in diff:
                        if db1_table.count(field) == 0:
                            ...
                            server = db1.get_host()
                        self.output("        - Field \"%s\" only exists on \"%s\"" % (field, server))
